%----------------------
%2.b Feature selection the right way
%----------------------
dim = 5000; % dimension
n_train = 100;
n_test = 100;
...
for real_idx = 1:n_realization
    ...
    %Generate train set x, test set x
    ...
    V= zeros(dim,1);
    for idx = 1:dim
        A = corrcoef(x_train(:,idx), y_train);
        V(idx) = abs(A(1,2));
    end
...
%    mse = sqrt(sum(y_test-y_test_pred).^2);
%    result(real_idx,1) = mse;
end

figure;
boxplot(result(:,:),{'correct'});
ylabel('');
xlabel('Testing error');
title(['Feature selection the right way, err = ',num2str(err)]);